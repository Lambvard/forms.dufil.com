...
//Staffid,stafffullname,customername,drivername,vehiclenumber,WayBillnumber,Invoice,staff_location,status_now,date_now,time_now,day_now,mon_now,year_now
		...
		$save_user_record="INSERT INTO liquidation.dbo.Ititemrequest (staffid,fullname,vname,vcompany,vexit,staff_location,status_now,date_now,time_now,day_now,mon_now,year_now,trans_id) values (?,?,?,?,?,?,?,?,?,?,?,?,?)";

		$prep=sqlsrv_prepare($db_connection,$save_user_record,array($staffid,$stafffullname,$vname,$vcompany,$vexit,$staff_location,$status_now,$date_now,$time_now,$day_now,$mon_now,$year_now,$new_id));
		if(sqlsrv_execute($prep)===true){
