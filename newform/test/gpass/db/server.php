...
elseif (isset($_POST['saveindividualrecordgatepassone'])) {
		...
		$year_now=date("Y");
			...
			$fequent_save=sqlsrv_query($db_connection,"SELECT count(*) as lambda_count FROM liquidation.dbo.stafftableliquidationtempdatagatepass where bindedid='$transactionallid'");
		...
		$save_frequent_record="INSERT INTO liquidation.dbo.stafftableliquidationtempdatagatepass (description,amount,specid,bindedid,dateofsubmit,timesubmit,op_day,op_mon,op_year) values (?,?,?,?,?,?,?,?,?)";
...
//Staffid,stafffullname,customername,drivername,vehiclenumber,WayBillnumber,Invoice,staff_location,status_now,date_now,time_now,day_now,mon_now,year_now
		...
		//$all_record=array($staffiduse,$staff_fullname,$staff_advance,$staff_cashrequest,$staff_location,$date_now,$time_now);
			...
			$pick_last_exp=sqlsrv_query($db_connection,"SELECT count(*) as lambda_count FROM liquidation.dbo.staffgatepasslog where staff_location='$staff_location' and mon_now='$mon_now'");
			$pick_exp=sqlsrv_fetch_array($pick_last_exp,SQLSRV_FETCH_ASSOC);
			//var_dump($pick_exp);

			$new_id=$staff_location."/".$date_now."/GPASS-".($pick_exp['lambda_count']+1);

				//$new_id=$pick_exp['lambda_count'];



		$save_user_record="INSERT INTO liquidation.dbo.staffgatepasslog (staffid,fullname,customername,drivername,vehiclenumber,WayBillnumber,Invoice,staff_location,status_now,date_now,time_now,day_now,mon_now,year_now,trans_id) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

		$prep=sqlsrv_prepare($db_connection,$save_user_record,array($staffid,$stafffullname,$customername,$drivername,$vehiclenumber,$WayBillnumber,$Invoice,$staff_location,$status_now,$date_now,$time_now,$day_now,$mon_now,$year_now,$new_id));
		if(sqlsrv_execute($prep)===true){
...
elseif(isset($_POST['deleteid'])){
	...
	$sql_del="DELETE from liquidation.dbo.stafftableliquidationtempdata WHERE specid=?";
