...
elseif (isset($_POST['saveindividualrecord'])) {
...
elseif(isset($_POST['savetransactiondetails'])){
		...
		//$all_record=array($staffiduse,$staff_fullname,$staff_advance,$staff_cashrequest,$staff_location,$date_now,$time_now);
			...
			$new_id=$staff_location."/".$date_now."/LIQ-".($pick_exp['lambda_count']+1);
				...
				//$new_id=$pick_exp['lambda_count'];
		$save_user_record="INSERT INTO liquidation.dbo.staffliquationexp (staffid,fullname,advanceiou,cashrequest,dateofsubmit,timesubmit,location,op_status,transid,op_day,op_mon,op_year) values (?,?,?,?,?,?,?,?,?,?,?,?)";

		$prep=sqlsrv_prepare($db_connection,$save_user_record,array($staffiduse,$staff_fullname,$staff_advance,$staff_cashrequest,$date_now,$time_now,$staff_location,$status_now,$new_id,$day_now,$mon_now,$year_now));
		if(sqlsrv_execute($prep)===true){
