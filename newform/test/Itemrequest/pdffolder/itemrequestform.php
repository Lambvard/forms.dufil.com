...
class babatunde extends FPDF{
	...
	function firstdetails($db_connection){
		...
		if(isset($_POST['ddd'])){
			...
			//and op_status=?
			$ref_num="";
			$sql_pull="SELECT * FROM liquidation.dbo.itemrequestform where trans_id=? ";
			...
			$sql_pull="SELECT count(*) AS ttamount FROM liquidation.dbo.itemrequestformlog where bindedid=?";
			$sql_pull_prep=sqlsrv_prepare($db_connection,$sql_pull,array($transaction_id));
			sqlsrv_execute($sql_pull_prep);
			$count_list_2=sqlsrv_fetch_array($sql_pull_prep,SQLSRV_FETCH_ASSOC);
			

			$sql_pull_sum="SELECT sum(CAST (amount AS INT)) AS sumup FROM liquidation.dbo.itemrequestformlog where bindedid=?";
...
//$company['companyname']
			...
			$this->Cell(0,10,"ITEM REQUEST FORM",0,0,"C");
			$this->LN(7);
		...
		/*	$this->Cell(0,10,"Way Bill Number:........................................................................................................................................................................................",0,0,"L");
			...
			*/
			...
			$this->Cell(-230,9,$count_list['remarks'],0,0,"C");
			$this->LN(15);
		...
		$sql_pull_all_s="SELECT * FROM liquidation.dbo.itemrequestformlog where bindedid='$transaction_id'";
	...
	//		for($i=1;$i<=$count_list_2['ttamount']; $i++){
			...
			$this->Cell(8,4,$number_trans,1,"C");
			$this->Cell(150,4,$count_list_3['description'],1,"L");
			...
			$this->Cell(-12,10,$count_list_sum['sumup'],0,0,"R");
			

			$this->LN(12);
			$this->setFont("Arial","B",9);
			$this->Cell(0,10,"FOR: DUFIL PRIMA",0,0,"L");
			//$this->Cell(0,10,"FOR: MULTIPRO",0,0,"R");
