...
class babatunde extends FPDF{
	...
	/*$find_location=sqlsrv_query($db_connection,"SELECT * FROM dbo.staff_transactionallogloan where loanidtrack='$user_transaction_id'");
		...
		if($current_details_pick['stafflocation']=="OTA-NOODLES"){
			$this->Cell(0,5,"REF. NO:  FRM/HR/001",0,0,"R");
		}
		...
		$this->Cell(0,0,"LAND /HOUSE OWNERSHIP SCHEME (SENIOR/JUNIOR)",0,0,"C");
		$this->LN(15);
	...
	//	$this->Cell(-200,5,$current_details_pick['paymentdur'],0,0,"C");
		...
		$this->LN(10);
		$this->Cell(0,5,"Originals of all documents will be required for Sighting.",0,0,"L");
		//$this->Cell(-40,3,$current_details_pick['reason'],0,0,"R");
	//	$this->Cell(-200,5,$current_details_pick['paymentdur'],0,0,"C");

		$this->LN(8);
		$this->Cell(0,5,"If answers to (Do you have a Land ?) is 'No', details must be submitted to Dufil Group management not later than 4 weeks",0,0,"L");
		$this->LN(5);
		$this->Cell(0,5,"of purchase of the land for which this application is meant.",0,0,"L");
		...
		$this->setFont("Arial","",10);
		$this->LN(8);
		$this->Cell(0,5,"1.	Loan repayable within 60 months of its grant.",0,0,"L");
		$this->LN(8);
		$this->Cell(0,5,"2.	7% flat interest rate to all qualified senior/junior staff.",0,0,"L");
		$this->LN(8);
		$this->Cell(0,5,"3.	Dufil Group is indemnified against any loss of property and/or life.",0,0,"L");
		$this->LN(8);
		$this->Cell(0,5,"4.	The land/house under the scheme is not transferable unless the loan is settled in full.",0,0,"L");
		...
		$this->LN(10);
		$this->setFont("Arial","",10);
		$this->Cell(0,5,"THIS AGREEMENT is made the-------------- day of ---------------         BETWEEN DUFIL PRIMA FOODS PLC ",0,0,"L");
		$this->Cell(-95,2,$current_details_pick['dayorig']."                    ".$current_details_pick['monthoftrans']." ".$current_details_pick['yearoftrans'],0,0,"R");
		$this->LN(8);
		
		$this->Cell(0,0,$currentloan_loc['companyaddress']."   (Hereinafter called the 'Company') of the one part and",0,0,"L");
	...
	//	$this->Cell(-200,5,$current_details_pick['guarantor1name'],0,0,"C");
		...
		$this->LN(7);
		$this->Cell(0,0,"3  The Loan shall be for a period of 60 months from the date of disbursement",0,0,"L");
		$this->LN(7);
		$this->Cell(0,0,"4. The employee shall pay interest on the loan at 7 (seven) percent flat interest rate.",0,0,"L");
		$this->LN(7);
		$this->Cell(0,0,"5. The Employee shall repay the loan and all sums due under this Agreement in 60 monthly installments to be deducted by",0,0,"L");
		$this->LN(7);
    	...
    	$this->LN(7);
    	$this->Cell(0,0,"DUFIL PRIMA FOODS PLC",0,0,"L");
    	$this->LN(7);
    	...
    	$this->LN(7);
    	$this->Cell(0,0,"Chief Finance Officer/Financial Controller                                                    														 								Factory Manager",0,0,"L");

    	$this->LN(15);
    	$this->Cell(0,0,"__________________________________________       																 						 														_______________________________",0,0,"L");
    	$this->LN(7);
    	$this->Cell(0,0,"Dir. HR Ext Rel/ HR & Admin Manager																																																																									Employee",0,0,"L");
		...
		$this->LN(7);
		$this->Cell(0,0,"DUFIL PRIMA FOODS PLC of (hereinafter called the 'Company' of the other part.",0,0,"L");
		//$this->LN(7);
		//$this->Cell(0,0,"18. The employee hereby agrees to forfeit his housing loan during the said period of repayment.",0,0,"L");
		$this->LN(10);
		$this->setFont("Arial","B",10);
...
//		$this->setFont("Arial","B",10);
		...
		$this->LN(20);
    	$this->Cell(0,0,"____________________________                                              _______________________________",0,0,"L");
    	$this->LN(7);
    	$this->Cell(0,0,"FINANCIAL CONTROLLER  	                                                                       DIRECTOR HR/EXT.RELS",0,0,"L");

    	$this->LN(20);
    	$this->Cell(0,0,"________________________                _______________________________          _________________________                                                    ",0,0,"L");
    	$this->LN(7);
    	$this->Cell(0,0,"CHIEF FINANCE OFFICER                        HUMAN RESOURCES MANAGER                  FACTORY MANAGER",0,0,"L");
    	...
    	$this->LN();
		$this->Cell(0,0,"Borrower) who applied for a loan of __________________________________from Dufil Prima Foods Plc",0,0,"L");
		$this->LN(7);
    	$this->Cell(320,0,$current_details_pick['fullname'],0,0,"C");
    	$this->LN(); 
		$this->Cell(0,0,"(DUFIL GROUP)(hereafter called the Company) WHEREAS MR./MRS./MISS  ___________________________________ ",0,0,"L");
		$this->LN(7);
    	$this->Cell(100,0,$current_details_pick['manamount'],0,0,"C");
    	$this->LN();
		$this->Cell(0,0," has applied for a loan of __________________________________  from the Company, and the Company has agreed ",0,0,"L");
		$this->LN(7); 
		...
		$this->LN(8);
    	$this->Cell(150,0,$current_details_pick['guarantor1position'],0,0,"C");
    	$this->LN(); 
		...
		$this->LN(8);
    	//$this->Cell(150,0,$current_details_pick[''],0,0,"C");
		...
		$this->LN(15);
    	$this->Cell(180,0,$current_details_pick['guarantor2name'],0,0,"C");
    	$this->LN(); 
		...
		$this->Cell(0,0,"Date: __________________________________________________________________________________________",0,0,"L");
		$this->LN(10); 
		$this->Cell(0,0,"HR/Admin Manager's signature and date:__________________________________________________",0,0,"L");
		$this->LN(20); 
