...
elseif (isset($_POST['saveitemssrf'])) {
		...
		$year_now=date("Y");
			...
			$fequent_save=sqlsrv_query($db_connection,"SELECT count(*) as lambda_count FROM liquidation.dbo.ititemrequestlog where bindedid='$transactionallid'");
		...
		$save_frequent_record="INSERT INTO liquidation.dbo.ititemrequestlog(itemname,serialnum,brand,model,amount,specid,bindedid,dateofsubmit,timesubmit,op_day,op_mon,op_year) values (?,?,?,?,?,?,?,?,?,?,?,?)";
		

		$prep=sqlsrv_prepare($db_connection,$save_frequent_record,array($vitemname,$vserial,$vbrand,$model,$amount,$new_id_save,$transactionallid,$date_now,$time_now,$day_now,$mon_now,$year_now));
...
elseif(isset($_POST['checksrf'])){
	...
	$selectionpurpose_value=$_POST['selectionpurpose_value'];
	$vexit=$_POST['vexit'];
	$vname=$_POST['vname'];
...
//Staffid,stafffullname,customername,drivername,vehiclenumber,WayBillnumber,Invoice,staff_location,status_now,date_now,time_now,day_now,mon_now,year_now
		...
		//$all_record=array($staffiduse,$staff_fullname,$staff_advance,$staff_cashrequest,$staff_location,$date_now,$time_now);
			...
			$pick_last_exp=sqlsrv_query($db_connection,"SELECT count(*) as lambda_count FROM liquidation.dbo.Ititemrequest where staff_location='$staff_location' and mon_now='$mon_now'");
			$pick_exp=sqlsrv_fetch_array($pick_last_exp,SQLSRV_FETCH_ASSOC);
			//var_dump($pick_exp);

			$new_id=$staff_location."/".$date_now."/ITITEMGATE-".($pick_exp['lambda_count']+1);

				//$new_id=$pick_exp['lambda_count'];



		$save_user_record="INSERT INTO liquidation.dbo.Ititemrequest (staffid,fullname,vname,vcompany,vexit,selectionpurpose_value,staff_location,status_now,date_now,time_now,day_now,mon_now,year_now,trans_id) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

		$prep=sqlsrv_prepare($db_connection,$save_user_record,array($staffid,$stafffullname,$vname,$vcompany,$vexit,$selectionpurpose_value,$staff_location,$status_now,$date_now,$time_now,$day_now,$mon_now,$year_now,$new_id));
		if(sqlsrv_execute($prep)===true){
		...
		$_SESSION['current_current']=$new_id;
			...
			$sql_qry="UPDATE liquidation.dbo.Ititemrequest SET status_now=? WHERE trans_id=? and status_now=?";
...
elseif(isset($_POST['deleteid'])){
	...
	$sql_del="DELETE from liquidation.dbo.ititemrequestlog WHERE specid=?";
