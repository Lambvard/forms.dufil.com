...
if(isset($_SESSION['current_current'])){
			...
			//destroy($_SESSION['current_current']);
	}
 ?>
}
...
//
		...
		//$('button[id^="id_r"').on('click','delete',function(e){
		//	e.preventDefault();
		//	var id=$(this).attr('name');
		//$(this).val();


								//$('rell').on('click','tr id_r.delete',function(e){
  								//e.preventDefault();
  								//$(this).closest('tr').remove();
  						
  				//console.log($('tr').find('id_r').attr('name'));
  				//console.log($('button').closest('rell').find('tr').find('id_r').attr('name'));//
