...
if(isset($_POST['saveorderdetails'])){
	$staffidnumber=$_POST['staffidnumber'];
	$userfullnames=$_POST['userfullnames'];
	$refnum=$_POST['refnum'];
	$dept_work=$_POST['dept_work'];
	$area_loc=$_POST['area_loc'];
	$equipfac=$_POST['equipfac'];
	$startdate=$_POST['startdate'];
}elseif (isset($_POST['checkstaffdetails'])) {
	
	$staffidnumber=$_POST['staffidnumber'];
	$pull_search=[];
	$st="Active";
	///S$search=array($staffidnumber,$st);
	$sql="SELECT * FROM dbo.staffdetails where Staffid=? and userstatus=?";
	$prepare_search=sqlsrv_prepare($db_connection,$sql,array($staffidnumber,$st));
	$execute_search=sqlsrv_execute($prepare_search);

	if($execute_search===true){
		$pull_search['fullname']=$prepare_search['surname']."  ".$prepare_search['firstname']."  ".$prepare_search['othernames'];
		$pull_search['stafflocationuse']=$prepare_search['stafflocation'];

		
	}//else{
		//$out= "notvalid";
		//echo json_encode($out);
		// $pull_search['err_r']="notvalid";
	//}
	echo json_encode($pull_search);
...
$save_leave_form=sqlsrv_query($db_connection,"SELECT * FROM dbo.staff_transactionallogcar where staffid='$staffidusecar' and status='$pending'");
$save_leave_form_count=sqlsrv_has_rows($save_leave_form);
	...
	if($con_user_2car>0){
		...
		header("Location:../index.php?car=norecord");
			...
			$temstatus_now="on";
		$user_query_logout=sqlsrv_query($db_connection,"SELECT * FROM dbo.staff_transactionallogcar WHERE staffid='$staffss' AND status='$temstatus_now'");
		$pick_user_details_logout=sqlsrv_has_rows($user_query_logout);
		if($pick_user_details_logout>0){
			$update=sqlsrv_query($db_connection,"UPDATE dbo.staff_transactionallogcar SET status='off' WHERE staffid='$staffss'");
			//session_destroy();
