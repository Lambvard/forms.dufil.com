...
if(isset($_POST['savedetails'])){
	...
	$userlocation=$_SESSION['locator'];
	$samplenumber=$_POST['samnumber'];
	$status="on";
...
$cont_track_2=sqlsrv_fetch_array($cont_track,SQLSRV_FETCH_ASSOC);
	...
	$insert_new_sensory_user=sqlsrv_query($db_connection,"INSERT INTO dbo.session_track (start_session,id_session,fullname,staff_id,dept,shift,end_session,userlocation,numsample,senstatus,datestart) values('$now_date','$new_id','$staff_name','$staff_id','$staff_dept','$staff_shift','null','$userlocation','$samplenumber','$status','$dt')");
...
*/
	...
	}elseif (isset($_POST['usertest'])) {
		$staffside=$_POST['stafftype'];
		$staffusedID=$_POST['stffid'];

		$find_user_type=sqlsrv_query($db_connection,"SELECT * from dbo.staffdetails where Staffid='$staffusedID' ");
		$find_user_type2=sqlsrv_has_rows($find_user_type);

		if($find_user_type2>0){
			//session_start();
			$_SESSION['iamin']=$staffusedID;
			header("Location:../fetcher/informationpages.php");

		}else{
			header("Location:../index.php?pivfh=errorlog");
		}
	}
